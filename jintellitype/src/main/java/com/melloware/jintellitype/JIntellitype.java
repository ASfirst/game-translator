...
//        loadDllFile();
        loadDllFileFromJavaPath();
        initializeLibrary();
        this.keycodeMap = getKey2KeycodeMapping();
    }

    protected void loadDllFile() {
        try {
            // Load JNI library
    ...
    protected void loadDllFileFromJavaPath() {
        String dllFileName1 = "JIntellitype.dll";
        String dllFileName2 = "JIntellitype64.dll";
        String arch = System.getProperty("os.arch");
        if (arch.contains("86")) {
            System.load(getClass().getResource("/" + dllFileName1).getFile());
        }
        else {
            System.load(getClass().getResource("/" + dllFileName2).getFile());
        }
    }
    ...
    public void registerHotKey(int identifier, String modifierAndKeyCode) {
        ...
        for (int i = 0; i < split.length; i++) {
            ...
            else if ("CTRL".equalsIgnoreCase(split[i]) || "CONTROL".equalsIgnoreCase(
                    split[i])) {
